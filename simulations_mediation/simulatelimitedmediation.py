...
def SimulatePhenotypes(mG,miID,iRun,rng):
    ...
    # generate mediators: with and without VarG(M)
    vM = mX@vBetaM + vG*dGM + vGS*dGSM + vES*dESM
    vM_A0 = mX@vBetaM + vES*dESM
    # generate outcomes: a and b non-zero, a=0, b=0
    vY = mX@vBetaY + vM*dB + vG*dGY + vE*dEY
    ...
    dfY_A0 = pd.DataFrame(mY_A0, index = miID, columns = lPheno)
    dfY_B0 = pd.DataFrame(mY_B0, index = miID, columns = lPheno)
    dfY_A0B0 = pd.DataFrame(mY_A0B0, index = miID, columns = lPheno)
    ...
    sFile_A0 = 'run.' + str(iRun) + '.A0.pheno.txt'
    sFile_B0 = 'run.' + str(iRun) + '.B0.pheno.txt'
    sFile_A0B0 = 'run.' + str(iRun) + '.A0B0.pheno.txt'
    sFile_covar = 'run.' + str(iRun) + '.covar.txt'
    # write phenotypes and covariates to tab separated files
    dfY.to_csv(sFile, sep='\t')
    dfY_A0.to_csv(sFile_A0, sep='\t')
    dfY_B0.to_csv(sFile_B0, sep='\t')
    dfY_A0B0.to_csv(sFile_A0B0, sep='\t')
