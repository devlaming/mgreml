...
|  | stratification) and for how many trailing PCs to |
...
|  | fields, followed by a field per phenotype; can be |
...
|  | IID as first two fields, followed by a field per |
|  | covariate; can be followed by optional flag |
|  | nolabelcovar, e.g. --covar mycovar.txt nolabelcovar, |
...
|  | column per covariate; can be followed by optional |
|  | flags nolabelpheno and/or nolabelcovar, but we |
...
|  | tab-separated, with one row per phenotype and one |
|  | column per environment factor; can be followed by |
|  | optional flags nolabelpheno and/or nolabelfactor, but |
...
|  | space-, or tab-separated, with one row per phenotype |
|  | and one column per environment factor; can be followed |
